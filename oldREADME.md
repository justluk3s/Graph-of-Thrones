...
The document corpus used by the RAG pipeline is stored in `characters_text.json`. It was generated from the original structured `characters.json` dataset, whose character data comes from Joakim Skoog's open-source project [An API of Ice And Fire](https://github.com/joakimskoog/AnApiOfIceAndFire). The API provides structured information about characters from George R. R. Martin's *A Song of Ice and Fire* universe and the HBO *Game of Thrones* series.

During preprocessing, each JSON character record was converted into a natural-language document. A document represents a single character and combines the available attributes, including names, aliases, gender, culture, titles, allegiances, family relationships, referenced books, and television appearances, into readable text suitable for semantic retrieval.

This transformation improves embedding quality by presenting related attributes as coherent textual context rather than as isolated fields and JSON syntax. It therefore enables the embedding model to capture semantic associations more effectively and supports retrieval based on meaning, even when a query does not use the exact field names or values found in the source dataset.

The resulting corpus serves as the unstructured knowledge source for the RAG pipeline. It complements rather than duplicates the RDF Knowledge Graph: the corpus provides rich descriptive context for retrieval and answer generation, while the Knowledge Graph preserves explicit structured relationships used during reasoning.
